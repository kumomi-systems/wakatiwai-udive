...
/// A reference to the [`DriverIO`] currently in use.
...
pub enum DriverType {
  /// Used for drivers that read a file and start an OS.
  BOOT,
  /// Used for drivers that open a file system and return a file's contents.
  FS
}
  ...
  pub fn path(&self) -> Option<CString16> {
    match self.driver_type {
      Some(some) => {
        ...
        ret.push_str(
          match some {
            DriverType::BOOT  => { BOOT_DRIVER_DIRECTORY }
            DriverType::FS    => { FSYS_DRIVER_DIRECTORY }
          }
        );
        ret.push_str(cstr16!("\\"));
